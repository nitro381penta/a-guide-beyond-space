...
public class RileyWaveField : MonoBehaviour, IWaveField
{
    [Header("Volume")]
    ...
    [Header("Wave Motion")]
    public float waveAmplitudeX = 0.26f;
    public float waveFrequencyY = 2.0f;
    public float waveFrequencyZ = 0.55f;
    public float timeSpeed = 0.9f;

    public float secondaryAmplitudeX = 0.05f;
    public float secondaryFrequencyY = 4.8f;
    public float secondaryTimeSpeed = 0.5f;

    public float driftAmplitudeX = 0.02f;
    public float driftSpeed = 0.3f;

    [Header("Downward Feel")]
    public bool useBottomWeight = true;
    public float bottomWeightAtBottom = 1.35f;
    public float bottomWeightAtTop = 0.75f;

    public bool useYSag = true;
    public float ySagAmplitude = 0.12f;
    public float ySagSpeed = 0.35f;
    public float ySagDepthFrequency = 0.22f;
    ...
    void Awake()
    {
        ...
        if (logToConsole)
            Debug.Log("[RileyWaveField] Awake -> line field created");
    }
    ...
    void HandleAudioOnStop()
    {
        ...
        if (waterAudioSource != null)
            StartWaterFade(0f, stopAfterFade: true);
    }
    ...
    void UpdateLine(WaveLine wl, float zSlice, float timeNow)
    {
        ...
        float baseX = Mathf.Lerp(-volumeWidth * 0.5f, volumeWidth * 0.5f, lineT);
        ...
        {
            float ty = p / (float)(pointsPerLine - 1);
            float y = Mathf.Lerp(-volumeHeight * 0.5f, volumeHeight * 0.5f, ty);

            float bottomWeight = 1f;
            if (useBottomWeight)
                bottomWeight = Mathf.Lerp(bottomWeightAtBottom, bottomWeightAtTop, ty);

            float primary =
                Mathf.Sin(y * waveFrequencyY + zSlice * waveFrequencyZ + timeNow * timeSpeed + phase)
                * waveAmplitudeX;

            float secondary =
                Mathf.Sin(y * secondaryFrequencyY - zSlice * 0.65f + timeNow * secondaryTimeSpeed + phase * 1.6f)
                * secondaryAmplitudeX;

            float drift =
                Mathf.Sin(timeNow * driftSpeed + zSlice * 0.18f + phase) * driftAmplitudeX;

            float x = baseX + (primary + secondary + drift) * bottomWeight;

            if (useYSag)
            {
                float ySag = Mathf.Sin(zSlice * ySagDepthFrequency + timeNow * ySagSpeed + phase)
                             * ySagAmplitude * (1f - ty);
                y -= ySag;
            }
    ...
    Gradient CreateDefaultPastelGradientA()
    {
        ...
        g.SetKeys(
            new[]
            {
                new GradientColorKey(new Color(0.76f, 0.84f, 0.96f), 0.00f),
                new GradientColorKey(new Color(0.83f, 0.92f, 0.78f), 0.30f),
                new GradientColorKey(new Color(0.97f, 0.86f, 0.69f), 0.65f),
                new GradientColorKey(new Color(0.90f, 0.80f, 0.95f), 1.00f)
            },
            new[]
            {
    ...
    Gradient CreateDefaultPastelGradientB()
    {
        ...
        g.SetKeys(
            new[]
            {
                new GradientColorKey(new Color(0.95f, 0.78f, 0.82f), 0.00f),
                new GradientColorKey(new Color(0.78f, 0.88f, 0.96f), 0.35f),
                new GradientColorKey(new Color(0.84f, 0.95f, 0.86f), 0.70f),
                new GradientColorKey(new Color(0.96f, 0.88f, 0.75f), 1.00f)
            },
            new[]
            {
                new GradientAlphaKey(1f, 0f),
    ...
    Gradient CreateDefaultPastelGradientC()
    {
        ...
        g.SetKeys(
            new[]
            {
                new GradientColorKey(new Color(0.88f, 0.82f, 0.96f), 0.00f),
                new GradientColorKey(new Color(0.79f, 0.93f, 0.90f), 0.28f),
                new GradientColorKey(new Color(0.97f, 0.83f, 0.73f), 0.62f),
                new GradientColorKey(new Color(0.78f, 0.84f, 0.98f), 1.00f)
            },
            new[]
            {
                new GradientAlphaKey(1f, 0f),
